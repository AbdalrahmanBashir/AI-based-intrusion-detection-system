...
RoutingProtocol::RoutingProtocol()
    : m_rreqRetries(2),
      ...
      m_ttlThreshold(7),
      m_timeoutBuffer(2),
      m_rreqRateLimit(10),               // 10
      m_rerrRateLimit(10),               // 10
      m_activeRouteTimeout(Seconds(31)), // 3
      m_netDiameter(3),                  // 35
      ...
      m_myRouteTimeout(Time(
          2 *
          std::max(m_pathDiscoveryTime,
                   m_activeRouteTimeout))), // m_myRouteTimeout(Time(2 *
      ...
      m_blackListTimeout(
          Time(m_rreqRetries *
               m_netTraversalTime)), // m_blackListTimeout(Time(m_rreqRetries * m_netTraversalTime))
      m_maxQueueLen(64),
...
{
    m_rreqSentCount = 0;
    m_rreqReceivedCount = 0;
    m_rrepSentCount = 0;
    m_rrepReceivedCount = 0;
    m_rerrSentCount = 0;
    m_rerrReceivedCount = 0;
                            ...
                            ErrorCallback ecb)
{
    ...
    // ToDo Malicious Attack should drop the packet
    if (IsMalicious)
    {
        std::cout << "BlackHole attack in the wirless network packet dropped" << std::endl;
        // Drop(p, header, Socket::ERROR_NOROUTETOHOST);
        // SendRerrForPacket(header);
        return false;
    }
    if (m_routingTable.LookupRoute(dst, toDst))
    {
        if (toDst.GetFlag() == VALID)
        {
            ...
            // std::cout << route->GetSource() << " forwarding to " << dst << " from " << origin
            //  << " packet " << p->GetUid()<<std::endl;
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (IsMalicious || m_routingTable.LookupRoute(dst, toDst))
    {
             ...
             toDst.GetValidSeqNo()))
        {
            ...
            {
                m_routingTable.LookupRoute(origin, toOrigin);
                // CODE ADDED BY ABDALRAHMAN BASHIR
                if (IsMalicious)
                {
                    Ptr<NetDevice> dev =
                    ...
                    RoutingTableEntry falseToDst(
                        ...
                        /*destination*/ dst,
                        /*validSeqNo*/ true,
                        /*seqNo*/ rreqHeader.GetDstSeqno() + 100,
                        /*interface*/
                        m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
                        /*hop*/ 1,
                        /*nextHop*/ dst,
